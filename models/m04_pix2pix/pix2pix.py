...
"""pix2pix gan，copy from https://github.com/affinelayer/pix2pix-tensorflow"""
...
"""collections.namedtuple 可以理解为 c 中的 struct，首先给每个 item 命名，然后给每个item 进行赋值"""
...
def save_images(fetches, step=None):
    ...
    filesets = []
    for i, in_path in enumerate(fetches["paths"]):  # batch 中的所有图片
        name, _ = os.path.splitext(os.path.basename(in_path.decode("utf8")))
        fileset = {"name": name, "step": step}
        for kind in ["inputs", "outputs", "targets"]:  # 三种图片：输入模态1图片，生成模态2图片，真实模态2图片
